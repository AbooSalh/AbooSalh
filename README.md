<div align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=30&duration=2800&pause=1500&color=10B981&center=true&vCenter=true&width=980&lines=Ahmed+Saleh+%7C+Software+Engineer;TypeScript%2FNode.js%2C+React+%26+Next.js;Building+Scalable+Web+Systems;C%23+%26+.NET+Developer" alt="Typing SVG" />
  <br/>
  <sub><i>Full-stack engineer shipping production systems with TypeScript, React, Node.js, and .NET</i></sub>
</div>

---

## Experience

**Full-Stack Developer** — Paradise Sharm Tours (Jan 2025 – Present)
- Architected and deployed a multi-language tour booking platform serving **174 MAU** across 6 countries with Next.js, Express.js, and MongoDB
- Built full observability stack (Prometheus, Grafana, k6) and achieved **91 Performance, 100 Accessibility** Lighthouse scores
- Implemented RBAC, JWT refresh token rotation, rate limiting, and automated daily backups

## Skills

TypeScript · C# · .NET · Node.js · Express · React · Next.js · MongoDB · PostgreSQL · Docker · Prometheus · Grafana

## Contact

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?logo=linkedin&logoColor=white)](https://linkedin.com/in/ahmed-saleh-swe/)
[![GitHub](https://img.shields.io/badge/GitHub-181717?logo=github&logoColor=white)](https://github.com/AbooSalh)
[![Portfolio](https://img.shields.io/badge/Portfolio-10B981?logo=google-chrome&logoColor=white)](https://abosaleh.site)
[![Email](https://img.shields.io/badge/Email-EA4335?logo=gmail&logoColor=white)](mailto:a.saleh.dev@gmail.com)
